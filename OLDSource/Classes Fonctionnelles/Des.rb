...
# #######################################################################
#
# Roulement normal : Des.roll(string)
# 		Formats : (string roulement) - ("20"), ("2d20"), ("3d6+5"), ("1d8-7")
#
# Garder les plus hauts : Des.highest(int,int,int)
# 		Formats : (int nbDes, int valeurDes, int modificateur) - (1, 6, 8), (2, 20, -5)
#
# Garder les plus bas : Des.lowest(int,int,int)
# 		Formats : (int nbDes, int valeurDes, int modificateur) - (1, 6, 8), (2, 20, -5)
#
# #######################################################################

# Test des variables externes
...
class Des
	...
	def Des.create(newMax)
		...
		if @@allDes.frozen?
			if $DEBUG
				puts 'DICE CREATION IS FROZEN : CAN\'T CREATE.'
			end
		...
		if exist?(newMax) != -1
			if $DEBUG
				puts 'DICE EXISTS. CAN\'T CREATE IT.'
			end
		...
		if $DEBUG
			puts 'DICE CREATION IS COMPLETED.'
		end
	end
	...
	def Des.roll(rouleDes)
		...
		if pos == rouleDes.length
			if $DEBUG
				puts 'Yd ROLL (' + nb.to_s + 'd' + valeur.to_s + ')'
			end
		...
		if $DEBUG
			puts 'Yd+Z ROLL (' + nb.to_s + 'd' + valeur.to_s + ' (' + modif.to_s + '))'
		end
	...
	def Des.minimum
		...
		if $DEBUG
			puts 'RETURNING MINIMUM (' + @@minimum.to_s + ')'
		end
	...
	def Des.highest(nb, valeur, keep)
		...
		if $DEBUG
			puts 'ROLLS FOR HIGHEST'
			puts rollArr
			puts 'KEEP ' + keep.to_s + ' HIGHEST = ' + total.to_s
		end
	...
	def Des.lowest(nb, valeur, keep)
		...
		if $DEBUG
			puts 'ROLLS FOR LOWEST'
			puts rollArr
			puts 'KEEP ' + keep.to_s + ' LOWEST = ' + total.to_s
		end
	...
	def Des.freezeCreation
		...
		if $DEBUG
			puts 'DICES CREATION IS NOW FROZEN'
		end
	end
	...
	def Des.exist?(trouveDes)
		...
		if $DEBUG
			puts 'd' + trouveDes.to_s + ' DOESN\'T EXISTS'
		end
...
if $DEBUG
	puts '-- CREATING EXISTING DICE TEST'
	Des.create(20)
end
...
if $DEBUG
	srand 1550
	puts '-- ROLL TESTS'
	puts '  -- STANDARD ROLL TEST (4d100-85)'
	Des.roll('4d100-85')
	puts '  -- HIGHEST ROLL TEST'
	Des.highest(4,6,3)
	puts '  -- LOWEST ROLL TEST'
	Des.lowest(5,8,2)
	puts '  -- MINIMUM DEACTIVATION AND ROLL 1d2-5'
	Des.setMinimum
	Des.roll('1d2-5')
	puts '  -- MINIMUM TO 5 AND ROLL 1d6'
	Des.setMinimum(5)
	Des.roll(6)
	puts '  -- MINIMUM TO 1 AND GET VALUE'
	Des.setMinimum(1)
	Des.minimum
